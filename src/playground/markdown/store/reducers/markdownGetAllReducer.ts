...
import { IMarkdownGetAll } from 'playground/markdown/classes/queries';
import { IMarkdown } from 'playground/markdown/classes/response';
import { Reducer } from 'redux';
import { MarkdownAction as Action } from '../actions';

const initialState: IQueryCollectionState<IMarkdownGetAll, IMarkdown> = {
  isExpired: false,
  isError: false,
  isLoading: false,
  request: undefined,
...
const reducer: Reducer<IQueryCollectionState<IMarkdownGetAll, IMarkdown>> = (state = initialState, action) => {
  switch (action.type) {
...
export { reducer as markdownGetAllReducer };