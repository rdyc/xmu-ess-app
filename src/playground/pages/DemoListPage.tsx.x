...
import { IProjectRegistrationListFilterResult, ProjectRegistrationListFilter } from './DemoListFilter';
...
const stateUpdaters: StateUpdaters<AllProps, IOwnState, IOwnStateUpdater> = {
  ...
  setConfig: () => (config: IListConfig<IProject>) => ({
    config
  }),
  setFilterVisibility: (prevState: IOwnState) => () => ({
    isFilterOpen: !prevState.isFilterOpen
  }),
  setFilterApplied: () => (filter: IProjectRegistrationListFilterResult) => ({
    ...filter,
    isFilterOpen: false
  }),
};
  ...
  componentDidMount() { 
    ...
    const config: IListConfig<IProject> = {
      ...
      onProcessSelection: (values: string[]) => {
        alert(`/playground/pages/demo/selected:${values}`);
      },
      ...
      actionComponent: (item: IProject) => (
        <React.Fragment>
          {/* {
            ...
            <Button 
              size="small"
              onClick={() => this.props.history.push(`/playground/pages/demo/single/${item.uid}`)}
            >
...
export const DemoListPage = compose(
  setDisplayName('DemoList'),
  withUser,
  withProjectRegistration,
  withRouter,
  injectIntl,
