...
import { KPIHRInputDetailView } from './KPIHRInputDetailView';
...
export type KPIHRInputDetailProps
  = WithUser
  & WithOidc
  & WithKPIEmployee
...
const createProps: mapper<KPIHRInputDetailProps, IOwnState> = (props: KPIHRInputDetailProps): IOwnState => {
...
const stateUpdaters: StateUpdaters<KPIHRInputDetailProps, IOwnState, IOwnStateUpdaters> = {
  setShouldLoad: (state: IOwnState, props: KPIHRInputDetailProps) => (): Partial<IOwnState> => ({
    shouldLoad: !state.shouldLoad
  }),
  setOptions: (prevState: IOwnState, props: KPIHRInputDetailProps) => (options?: IPopupMenuOption[]): Partial<IOwnState> => ({
    menuOptions: options
  }),
  setModify: (prevState: IOwnState, props: KPIHRInputDetailProps) => (): Partial<IOwnState> => ({
...
const handlerCreators: HandleCreators<KPIHRInputDetailProps, IOwnHandler> = {
  handleOnLoadApi: (props: KPIHRInputDetailProps) => () => { 
  ...
  handleOnSelectedMenu: (props: KPIHRInputDetailProps) => (item: IPopupMenuOption) => { 
    switch (item.id) {
  ...
  handleOnCloseDialog: (props: KPIHRInputDetailProps) => () => {
    props.setDefault();
  },
  handleOnConfirm: (props: KPIHRInputDetailProps) => () => {
...
const lifecycles: ReactLifeCycleFunctions<KPIHRInputDetailProps, IOwnState> = {
  componentDidUpdate(prevProps: KPIHRInputDetailProps) {
...
export const KPIHRInputDetail = compose(
  withRouter,
  withOidc,
  withUser,
  withKPIEmployee,
  ...
  setDisplayName('KPIHRInputDetail')
)(KPIHRInputDetailView);