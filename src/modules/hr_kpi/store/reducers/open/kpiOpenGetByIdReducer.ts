...
import { IKPIOpenGetDetailRequest } from '@kpi/classes/queries/open';
import { IKPIOpenDetail } from '@kpi/classes/response/open';
import { KPIOpenAction as Action } from '@kpi/store/actions';
import { Reducer } from 'redux';

const initialState: IQuerySingleState<IKPIOpenGetDetailRequest, IKPIOpenDetail> = {
  isExpired: false,
  isError: false,
  isLoading: false,
  request: undefined,
...
const reducer: Reducer<IQuerySingleState<IKPIOpenGetDetailRequest, IKPIOpenDetail>> = (state = initialState, action) => {
  switch (action.type) {
...
export { reducer as kpiOpenGetByIdReducer };