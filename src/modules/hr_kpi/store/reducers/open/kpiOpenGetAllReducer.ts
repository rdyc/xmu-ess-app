...
import { IKPIOpenGetAllRequest } from '@kpi/classes/queries/open';
import { IKPIOpen } from '@kpi/classes/response/open';
import { KPIOpenAction as Action } from '@kpi/store/actions';
import { Reducer } from 'redux';

const initialState: IQueryCollectionState<IKPIOpenGetAllRequest, IKPIOpen> = {
  isExpired: false,
  isError: false,
  isLoading: false,
  request: undefined,
...
const reducer: Reducer<IQueryCollectionState<IKPIOpenGetAllRequest, IKPIOpen>> = (state = initialState, action) => {
  switch (action.type) {
...
export { reducer as kpiOpenGetAllReducer };