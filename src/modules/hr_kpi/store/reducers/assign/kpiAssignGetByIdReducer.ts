...
import { IKPIAssignGetByIdRequest } from '@kpi/classes/queries';
import { IKPIAssignDetail } from '@kpi/classes/response';
import { KPIAssignAction as Action } from '@kpi/store/actions';
import { Reducer } from 'redux';

const initialState: IQuerySingleState<IKPIAssignGetByIdRequest, IKPIAssignDetail> = {
  isExpired: false,
  isError: false,
  isLoading: false,
  request: undefined,
...
const reducer: Reducer<IQuerySingleState<IKPIAssignGetByIdRequest, IKPIAssignDetail>> = (state = initialState, action) => {
  switch (action.type) {
...
export { reducer as kpiAssignGetByIdReducer };