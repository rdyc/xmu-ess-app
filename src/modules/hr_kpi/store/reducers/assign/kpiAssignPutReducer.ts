...
import { IKPIAssignPutRequest } from '@kpi/classes/queries';
import { IKPIAssign } from '@kpi/classes/response';
import { KPIAssignAction as Action } from '@kpi/store/actions';
import { Reducer } from 'redux';

const initialState: IQuerySingleState<IKPIAssignPutRequest, IKPIAssign> = {
  isExpired: false,
  isError: false,
  isLoading: false,
  request: undefined,
...
const reducer: Reducer<IQuerySingleState<IKPIAssignPutRequest, IKPIAssign>> = (state = initialState, action) => {
  switch (action.type) {
...
export { reducer as kpiAssignPutReducer };