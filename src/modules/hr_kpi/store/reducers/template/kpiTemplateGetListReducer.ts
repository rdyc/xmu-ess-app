...
import { IKPITemplateGetListRequest } from '@kpi/classes/queries';
import { IKPITemplate } from '@kpi/classes/response';
import { KPITemplateAction as Action } from '@kpi/store/actions';
import { Reducer } from 'redux';

const initialState: IQueryCollectionState<IKPITemplateGetListRequest, IKPITemplate> = {
  isExpired: false,
  isError: false,
  isLoading: false,
  request: undefined,
...
const reducer: Reducer<IQueryCollectionState<IKPITemplateGetListRequest, IKPITemplate>> = (state = initialState, action) => {
  switch (action.type) {
...
export { reducer as kpiTemplateGetListReducer };