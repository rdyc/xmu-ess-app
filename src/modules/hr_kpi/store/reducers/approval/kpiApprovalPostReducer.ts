...
import { IKPIApprovalPostRequest } from '@kpi/classes/queries';
import { IKPIEmployee } from '@kpi/classes/response';
import { KPIApprovalAction as Action } from '@kpi/store/actions';
import { Reducer } from 'redux';

const initialState: IQueryCollectionState<IKPIApprovalPostRequest, IKPIEmployee> = {
  isExpired: false,
  isError: false,
  isLoading: false,
  request: undefined,
...
const reducer: Reducer<IQueryCollectionState<IKPIApprovalPostRequest, IKPIEmployee>> = (state = initialState, action) => {
  switch (action.type) {
...
export { reducer as kpiApprovalPostReducer };