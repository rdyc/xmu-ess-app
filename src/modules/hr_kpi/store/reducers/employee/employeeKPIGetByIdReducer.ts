...
import { IEmployeeKPIGetByIdRequest } from '@kpi/classes/queries';
import { IEmployeeKPIDetail } from '@kpi/classes/response';
import { EmployeeKPIAction as Action } from '@kpi/store/actions';
import { Reducer } from 'redux';

const initialState: IQuerySingleState<IEmployeeKPIGetByIdRequest, IEmployeeKPIDetail> = {
  isExpired: false,
  isError: false,
  isLoading: false,
  request: undefined,
...
const reducer: Reducer<IQuerySingleState<IEmployeeKPIGetByIdRequest, IEmployeeKPIDetail>> = (state = initialState, action) => {
  switch (action.type) {
...
export { reducer as employeeKPIGetByIdReducer };