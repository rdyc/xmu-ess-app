...
function* watchAllRequest() {
  const worker = (action: ReturnType<typeof accountEmployeeNoteGetAllRequest>) => {
    return saiyanSaga.fetch({
      method: 'get',
      path: `/v1/account/employees/${action.payload.employeeUid}/notes${objectToQuerystring(action.payload.filter)}`, 
      successEffects: (response: IApiResponse) => ([
        put(accountEmployeeNoteGetAllSuccess(response.body)),
      ]), 
      failureEffects: (response: IApiResponse) => ([
        put(accountEmployeeNoteGetAllError(response.statusText)),
        put(layoutAlertAdd({
          time: new Date(),
...
function* watchListRequest() {
  const worker = (action: ReturnType<typeof accountEmployeeNoteGetListRequest>) => {
    return saiyanSaga.fetch({
      method: 'get',
      path: `/v1/account/employees/${action.payload.employeeUid}/notes/list${objectToQuerystring(action.payload.filter)}`,
      successEffects: (response: IApiResponse) => ([
        put(accountEmployeeNoteGetListSuccess(response.body))
      ]), 
      failureEffects: (response: IApiResponse) => ([
        put(accountEmployeeNoteGetListError(response.statusText)),
        put(layoutAlertAdd({
          time: new Date(),
          message: response.statusText,
          details: response
...
function* watchByIdRequest() {
  const worker = (action: ReturnType<typeof accountEmployeeNoteGetByIdRequest>) => {
    return saiyanSaga.fetch({
      ...
      failureEffects: (response: IApiResponse) => ([
        put(accountEmployeeNoteGetByIdError(response.statusText)),
        put(layoutAlertAdd({
          time: new Date(),
          message: response.statusText,
          details: response
  ...
  const worker = (action: ReturnType<typeof accountEmployeeNoteDeleteRequest>) => {
    return saiyanSaga.fetch({
      ...
      successEffects: (response: IApiResponse) => [
        ...
        put(accountEmployeeNoteGetAllRequest(response.body))
      ],
      successCallback: (response: IApiResponse) => {
        action.payload.resolve(response.body.data);
      },
