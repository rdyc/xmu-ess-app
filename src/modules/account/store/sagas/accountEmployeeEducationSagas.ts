...
function* watchAllRequest() {
  const worker = (action: ReturnType<typeof accountEmployeeEducationGetAllRequest>) => {
    return saiyanSaga.fetch({
      method: 'get',
      path: `/v1/account/employees/${action.payload.employeeUid}/educations${objectToQuerystring(action.payload.filter)}`, 
...
function* watchListRequest() {
  const worker = (action: ReturnType<typeof accountEmployeeEducationGetListRequest>) => {
    return saiyanSaga.fetch({
      method: 'get',
      path: `/v1/account/employees/${action.payload.employeeUid}/educations/list${objectToQuerystring(action.payload.filter)}`,
      successEffects: (response: IApiResponse) => ([
