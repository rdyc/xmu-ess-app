...
import { SettlementListProps } from './TravelSettlementList';

export const TravelSettlementListView: React.SFC<SettlementListProps> = props => {
  const { handleGoToDetail } = props;
  const { isLoading, response } = props.travelSettlementState.all;

  const renderSettlementList = (travels: ITravelSettlement[]) => {
    ...
    >
      {
        ...
        renderSettlementList(response.data)
      }
    </List>
  );
