...
const reducer: Reducer<IQuerySingleState<ISummaryGetProfitabilityRequest, ISummaryProfitability>> = (state = initialState, action) => {
  switch (action.type) {
    case Action.GET_PROFITABILITY_REQUEST: return { ...state, isLoading: true, request: action.payload };
    case Action.GET_PROFITABILITY_SUCCESS: return { ...state, isLoading: false, response: action.payload };
