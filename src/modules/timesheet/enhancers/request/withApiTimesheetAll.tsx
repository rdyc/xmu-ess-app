...
import { timesheetGetAllDispose, timesheetGetAllRequest } from '@timesheet/store/actions';
...
export interface WithApiTimesheetAllHandler {
  handleSync: () => void;
  handleNext: () => void;
  handlePrev: () => void;
...
interface WithApiTimesheetAllOptions {
...
interface Dispatcher {
  timesheetAllDispatch: {
    getAllRequest: typeof timesheetGetAllRequest;
    getAllDispose: typeof timesheetGetAllDispose;
  };
}
...
const withApiTimesheetAll = (options?: WithApiTimesheetAllOptions) => (WrappedComponent: React.ComponentType) => {
  const displayName = `WithApiTimesheetAll(${WrappedComponent.displayName || WrappedComponent.name || 'Component'})`;

  const withApiTimesheetAllWrapper: React.SFC<AllProps> = props => <WrappedComponent {...props} />;

  const createProps: mapper<AllProps, State> = (props: AllProps): State => {
    const { request } = props.timesheetAllState;
  ...
  const stateUpdaters: StateUpdaters<WithApiTimesheetAllOptions, State, Updaters> = {
  ...
  const handlerCreators: HandleCreators<AllProps, WithApiTimesheetAllHandler> = {
  ...
  const mapDispatchToProps = (dispatch: Dispatch) => ({
    timesheetAllDispatch: {
      getAllRequest: (request: ITimesheetGetAllRequest) => dispatch(timesheetGetAllRequest(request)),
      getAllDispose: () => dispatch(timesheetGetAllDispose()),
    }
  });

  const lifeCycleFunctions: ReactLifeCycleFunctions<AllProps, {}> = {
    componentDidMount() {
      const { response } = this.props.timesheetAllState;
    ...
    componentWillUnmount() {
      ...
      const { getAllDispose } = this.props.timesheetAllDispatch;
  ...
  const loadData = (props: AllProps): void => {
    ...
    const { getAllRequest } = props.timesheetAllDispatch;
  ...
  return compose<AllProps, WithApiTimesheetAllOptions>(
    setDisplayName(displayName),
    connect(undefined, mapDispatchToProps),
    withUser,
    withLayout,
    withTimesheetAll,
    withStateHandlers<State, Updaters, WithApiTimesheetAllOptions>(createProps, stateUpdaters), 
    withHandlers<AllProps, WithApiTimesheetAllHandler>(handlerCreators),
    lifecycle<AllProps, {}>(lifeCycleFunctions),
  )(withApiTimesheetAllWrapper);
};

export default withApiTimesheetAll;