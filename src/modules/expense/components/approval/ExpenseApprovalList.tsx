...
import withApiExpenseApprovalAll, { WithApiExpenseApprovalAllHandler } from '@expense/enhancers/approval/withApiExpenseApprovalAll';
...
const ApprovalList: React.SFC<AllProps> = props => {
  const { history } = props;
  const { isLoading, response } = props.expenseApprovalAllState;
  ...
  const renderExpenseApprovalList = (expenses: IExpense[]) => {
    ...
    >
      {
        ...
        renderExpenseApprovalList(response.data)
      }
    </List>
  );
