import { IExpensePostRequest } from '@expense/classes/queries';
import { IExpense } from '@expense/classes/response';
import { ExpenseAction as Action } from '@expense/store/actions';
import { IQuerySingleState } from '@generic/interfaces';
import { Reducer } from 'redux';

const initialState: IQuerySingleState<IExpensePostRequest, IExpense> = {
  request: undefined,
  response: undefined,
  isError: false,
  isLoading: false,
...
const reducer: Reducer<IQuerySingleState<IExpensePostRequest, IExpense>> = (state = initialState, action) => {
  switch (action.type) {
...
export { reducer as expensePostReducer };