...
const reducer: Reducer<IQuerySingleState<IExpenseApprovalGetByIdRequest, IExpenseDetail>> = (state = initialState, action) => {
  switch (action.type) {
    case Action.APPROVAL_GET_BY_ID_REQUEST: return { ...state, isLoading: true, isError: false, request: action.payload };
    case Action.APPROVAL_GET_BY_ID_SUCCESS: return { ...state, isLoading: false, isError: false, response: action.payload };
