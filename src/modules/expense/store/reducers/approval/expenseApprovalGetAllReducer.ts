...
const reducer: Reducer<IQueryCollectionState<IExpenseApprovalGetAllRequest, IExpense>> = (state = initialState, action) => {
  switch (action.type) {
    case Action.APPROVAL_GET_ALL_REQUEST: return { ...state, isExpired: false, isLoading: true, isError: false, request: action.payload };
    case Action.APPROVAL_GET_ALL_SUCCESS: return { ...state, isExpired: false, isLoading: false, isError: false, response: action.payload };
    case Action.APPROVAL_GET_ALL_ERROR: return { ...state, isExpired: false, isLoading: false, isError: true, errors: action.payload };
    case Action.APPROVAL_GET_ALL_DISPOSE: return { ...state, isExpired: true };
