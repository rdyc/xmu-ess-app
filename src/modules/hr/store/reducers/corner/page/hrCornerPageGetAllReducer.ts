...
import { IHrCornerPageGetAllRequest } from '@hr/classes/queries';
import { IHrCornerPage } from '@hr/classes/response';
import { HrCornerPageAction as Action } from '@hr/store/actions';
import { Reducer } from 'redux';

const initialState: IQueryCollectionState<IHrCornerPageGetAllRequest, IHrCornerPage> = {
  isExpired: false,
  isError: false,
  isLoading: false,
  request: undefined,
...
const reducer: Reducer<IQueryCollectionState<IHrCornerPageGetAllRequest, IHrCornerPage>> = (state = initialState, action) => {
  switch (action.type) {
...
export { reducer as hrCornerPageGetAllReducer };