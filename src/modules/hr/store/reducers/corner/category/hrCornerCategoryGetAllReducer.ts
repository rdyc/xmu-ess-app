...
import { IHrCornerCategoryGetAllRequest } from '@hr/classes/queries';
import { IHrCornerCategory } from '@hr/classes/response';
import { HrCornerCategoryAction as Action } from '@hr/store/actions';
import { Reducer } from 'redux';

const initialState: IQueryCollectionState<IHrCornerCategoryGetAllRequest, IHrCornerCategory> = {
  isExpired: false,
  isError: false,
  isLoading: false,
  request: undefined,
...
const reducer: Reducer<IQueryCollectionState<IHrCornerCategoryGetAllRequest, IHrCornerCategory>> = (state = initialState, action) => {
  switch (action.type) {
...
export { reducer as hrCornerCategoryGetAllReducer };