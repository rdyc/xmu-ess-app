...
import { IHrCornerBlogGetDetailRequest } from '@hr/classes/queries';
import { IHrCornerBlogDetail } from '@hr/classes/response';
import { HrCornerBlogAction as Action } from '@hr/store/actions';
import { Reducer } from 'redux';

const initialState: IQuerySingleState<IHrCornerBlogGetDetailRequest, IHrCornerBlogDetail> = {
  isExpired: false,
  isError: false,
  isLoading: false,
  request: undefined,
...
const reducer: Reducer<IQuerySingleState<IHrCornerBlogGetDetailRequest, IHrCornerBlogDetail>> = (state = initialState, action) => {
  switch (action.type) {
...
export { reducer as hrCornerBlogGetDetailReducer };