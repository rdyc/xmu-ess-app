...
import { IHrCompetencyClusterDeleteRequest } from '@hr/classes/queries';
import { HrCompetencyClusterAction as Action } from '@hr/store/actions';
import { Reducer } from 'redux';

const initialState: IQuerySingleState<IHrCompetencyClusterDeleteRequest, boolean> = {
  isExpired: false,
  isError: false,
  isLoading: false,
  request: undefined,
...
const reducer: Reducer<IQuerySingleState<IHrCompetencyClusterDeleteRequest, boolean>> = (state = initialState, action) => {
  switch (action.type) {
...
export { reducer as hrCompetencyClusterDeleteReducer };