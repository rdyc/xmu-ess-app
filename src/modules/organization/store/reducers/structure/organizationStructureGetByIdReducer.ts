...
const reducer: Reducer<IQuerySingleState<IOrganizationStructureByIdRequest, IStructureDetail>> = (state = initialState, action) => {
  switch (action.type) {
    case Action.GET_BY_ID_STRUCTURE_REQUEST: return { ...state, isLoading: true, isError: false, request: action.payload };
    case Action.GET_BY_ID_STRUCTURE_SUCCESS: return { ...state, isLoading: false, isError: false, response: action.payload };
